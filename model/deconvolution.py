...
class Deconvolution(torch.nn.Module):
    def __init__(self, shellSampling,graphSampling, filter_start, kernel_sizeSph, kernel_sizeSpa, n_equi, n_inva, normalize, conv_name, isoSpa, feature_in,full_basis,sh_degree):
    ...
    def forward(self, x):
        ...
        x_deconvolved_equi, x_deconvolved_inva = self.separate(x_deconvolved) # B x out_channels_equi x V x X x Y x Z, B x out_channels_inva x V x X x Y x Z
