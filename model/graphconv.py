...
class SO3SE3Conv(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_sizeSph, kernel_sizeSpa, bias=True, isoSpa=True):
        ...
        if self.isoSpa:
            ...
            shape_center = (out_channels, in_channels, 1, 1, 1)
        else:
        ...
        self.weightSpa_center = torch.nn.Parameter(torch.Tensor(*shape_center))
        if bias:
            self.bias = torch.nn.Parameter(torch.Tensor(out_channels))
        else:
            self.register_parameter("bias", None)
    ...
    def kaiming_initialization(self):
        ...
        self.weightSpa_center.data.normal_(0,std)
        if self.bias is not None:
            self.bias.data.fill_(0.01)
    ...
    def forward(self, laplacian, inputs):
        ...
        if self.isoSpa:
            ...
            outputs = self._conv(laplacian, inputs, self.weightSph, weight,self.weightSpa_center)
        else:
            outputs = self._conv(laplacian, inputs, self.weightSph, self.weightSpa,self.weightSpa_center)
        if self.bias is not None:
            outputs += self.bias[None, :, None, None, None, None]
        return outputs


def se3so3_conv(laplacian, inputs, weightSph, weightSpa,weightSpa_center):
    ...
    Fout, Fin, kX_center, kY_center, kZ_center = weightSpa_center.shape
    # B = batch size
    # V = nb vertices
    # Fin = nb input features
    # Fout = nb output features
    ...
    wSph_center = weightSph.view([Fout, Fin*K, 1, 1, 1]).expand(-1, -1, kX_center, kY_center, kZ_center) # Fout x Fin*K x kX x kY x kZ
    wSpa_center = weightSpa_center.repeat_interleave(K, dim=1) # Fout x Fin*K x kX x kY x kZ
    weight_center = wSph_center * wSpa_center 
    
    outputs = torch.nn.functional.conv3d(inputs, weight, padding='same') # B*V x Fout x X x Y x Z
    outputs_center = torch.nn.functional.conv3d(inputs, weight_center, padding='same') # B*V x Fout x X x Y x Z
    outputs_all = outputs+outputs_center
    # Get final output tensor
    outputs_all = outputs_all.view([B, V, Fout, X, Y, Z])  # B x V x Fout x X x Y x Z
    outputs_all = outputs_all.permute(0, 2, 1, 3, 4, 5).contiguous()  # B x Fout x V x X x Y x Z
