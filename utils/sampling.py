...
class HealpixSampling:
    ...
    """
    def __init__(self, n_side, depth, patch_size,full_basis, sh_degree=None, pooling_mode='average', pooling_name='mixed'):
        ...
        self.sampling = Sampling(G.coords, full_basis,sh_degree)
        # print(self.sampling.S2SH.shape[1], (sh_degree+1)*(sh_degree//2+1))  #45 45
...
class BvecSampling:
    """Graph Spherical sampling class.
    """
    def __init__(self, bvec, depth, image_size, full_basis,sh_degree=None, pooling_mode='average'):
        """Initialize the sampling class.
        Args:
            n_side (int): Healpix resolution
            depth (int): Depth of the encoder
        ...
        if full_basis==True:
            assert self.sampling.S2SH.shape[1] == (sh_degree+1)*(sh_degree+1)
        else:
            assert self.sampling.S2SH.shape[1] == (sh_degree+1)*(sh_degree//2+1)
...
class ShellSampling:
    ...
    """
    def __init__(self, full_basis,vectors_path, shell_path, sh_degree=None, max_sh_degree=None):
        """Initialize the sampling class.
        Args:
        ...
        shell_0_index = np.where(shell!=0)[0]
        shell_ = shell[shell_0_index]
       
        if shell_.ndim==0:
            shell_ = np.expand_dims(shell_, axis=-1)
       
         # Load sampling
        vectors = np.loadtxt(vectors_path)
        vectors = vectors[:,shell_0_index]
        ...
        assert shell_.shape[0] == vectors.shape[0]
        ...
        for s in self.shell_values:
            ...
            s_sampling = Sampling(vertice,full_basis, sh_degree, max_sh_degree, s==0)
...
class Sampling:
    ...
    def __init__(self,vectors,full_basis, sh_degree=None, max_sh_degree=None, constant=False, symmetric=False):
        ...
        if constant:
            ...
            if full_basis==True:
                self.SH2S = np.zeros((int((sh_degree+1)*(sh_degree+1)), vectors.shape[0]))
            if full_basis==False:
                self.SH2S = np.zeros((int((sh_degree+1)*((sh_degree+2)/2)), vectors.shape[0]))
            self.SH2S[0] = 1 / math.sqrt(4*math.pi)
        else:
            # Compute SH matrices
            _, self.SH2S = self.sh_matrix(sh_degree, vectors, full_basis,with_order=1) # (sh_degree+1)(sh_degree//2+1) x V 
            ...
            self.S2SH, _ = self.sh_matrix(sh_degree_s2sh, vectors, full_basis,with_order=1) # V x (sh_degree_s2sh+1)(sh_degree_s2sh//2+1)

    def sh_matrix(self, sh_degree, vectors,full_basis, with_order):
        return _sh_matrix(sh_degree, vectors,full_basis, with_order)

def _sh_matrix(sh_degree, vector, full_basis,with_order=1):
    """
